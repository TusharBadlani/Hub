...
class ConnectionLiveData @Inject constructor(
        ...
        override fun onAvailable(network: Network) {
            ...
            if (hasInternetCapability == true) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    val hasInternet = testInternetConnectivity(network.socketFactory)
                    if (hasInternet) {
    ...
    /**
     * Runs a PING request to google's server to test internet connectivity of
     * a network.
     *
     * @return true if the PING was successful, false otherwise
     */
    private fun testInternetConnectivity(socketFactory: SocketFactory): Boolean {
        return try {
